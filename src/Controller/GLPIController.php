...
class GLPIController extends AbstractController
{
...
//            dump($e); exit;
    ...
    /* Ajout note sur ticket */
    public function addTicketFollowup($id, $note)
    {
        $data = [
            'users_id' => 180, //$_SESSION['idGLPI'],
            'items_id' => $id,
            'content' => $note,
            'itemtype' => 'Ticket',
            'requesttypes_id' => 2,
        ...
        try {
            $this->connect();
            $response = $this->client->request('POST', 'ITILFollowup',
                [
                    'headers' => $this->getHeaders('application/json'),
                    'json' => [
                        'input' => $data
    ...
    /* Ajout document */
    public function addDocument($id, $file)
    {
//        try {
        $this->connect();
        $response = $this->client->request('POST', 'Ticket/'.$id.'/Document_Item',
            [
                'headers' => $this->getHeaders('application/json'),
                'json' => [
                    'input' => [
